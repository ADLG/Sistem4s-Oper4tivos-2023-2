...
process_execute (const char *file_name) 
{
  ...
  sema_init(&t->cargado,0);
  sema_down(&t->cargado);

  /* Los casos para controlar los errores con el tid y si no esta listo el hilo hijo.  */
  if (tid == TID_ERROR)
...
start_process (void *file_name_)
{
  ...
  struct thread * t = thread_current();
  sema_up(&t->padre->cargado);

  /* Si success es falso se indica que el hilo no se ha cargado correctamente y se hace un llamado a thread_exit(), 
  en caso contrario se sigue con el flujo normal de la ejecución. */
  if (!success)
  {
    t->padre->thready = false;
    thread_exit ();
  }
  else
  {
...
process_wait (tid_t child_tid UNUSED) 
{
  ...
  if (!list_empty(hijos))
  {
    for (hijos_e = list_front(hijos); hijos_e != list_end(hijos); hijos_e = list_next(hijos_e))
    {
      ...
      if (thhijo->tid == child_tid)
      {
        sema_down(&t->wait);
        list_remove(hijos_e);
        return thhijo->exitst;
      }
    }
  }
  return -1;
